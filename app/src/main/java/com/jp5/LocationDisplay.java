...
public class LocationDisplay extends CamActivity {
	...
	public void onCreate( final Bundle savedInstanceState ) {
        ...
        Log.i("debug", "SampleCamContentFromNativeActivity");
		...
		this.locationListener = new LocationListener() {
			...
			public void onLocationChanged( final Location location ) {
				if (location!=null) {
					LocationDisplay.this.lastKnownLocaton = location;
				if ( LocationDisplay.this.architectView != null ) {
					if ( location.hasAltitude() ) {
                        LocationDisplay.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy() );
					} else {
                        LocationDisplay.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAccuracy() );
					}
				}
				}
			}
	...
	final Runnable loadData = new Runnable() {
		...
		public void run() {
            ...
            LocationDisplay.this.isLoading = true;
			
			final int WAIT_FOR_LOCATION_STEP_MS = 2000;
			
			while (LocationDisplay.this.lastKnownLocaton==null && !LocationDisplay.this.isFinishing()) {

                LocationDisplay.this.runOnUiThread(new Runnable() {
					
					@Override
					public void run() {
						Toast.makeText(LocationDisplay.this, R.string.location_fetching, Toast.LENGTH_SHORT).show();
					}
				});
			...
			if (LocationDisplay.this.lastKnownLocaton!=null && !LocationDisplay.this.isFinishing()) {
				// TODO: you may replace this dummy implementation and instead load POI information e.g. from your database
                LocationDisplay.this.poiData = LocationDisplay.getPoiInformation(LocationDisplay.this.lastKnownLocaton, 20);
                try {
                    Log.i("Location:", LocationDisplay.this.poiData.toString(4));
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                LocationDisplay.this.callJavaScript("World.loadPoisFromJsonData", new String[] { LocationDisplay.this.poiData.toString() });
			}


            LocationDisplay.this.isLoading = false;
		}
	};
